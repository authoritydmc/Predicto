...
class SimpleConsoleLogger:
    ...
    def __init__(self, name="FixedWindowsOrchestrator"):
        self.name = name
...
class FixedWindowsOrchestrator:
    """
    Fixed Windows orchestrator with proper task management
    """
    ...
    def _setup_windows_ipc(self):
        ...
        try:
            import win32event
            self._ipc_event = win32event.CreateEvent(None, False, False, f"FixedOrchestrator_{self.session_id}")
            self.logger.info(f"Windows IPC Event created: FixedOrchestrator_{self.session_id}")
        except ImportError:
    ...
    async def start(self):
        """Start the fixed Windows orchestrator"""
        self.running = True
        ...
        self.logger.info(f"Starting Fixed Windows Orchestrator (Session: {self.session_id})")
        ...
        self.logger.info("Fixed Windows Orchestrator started successfully")
                        ...
                        with self._execution_lock:
                            ...
                            try:
                                ...
                                if "no running event loop" in str(re):
                                    print(f"[FixedWindowsOrchestrator] Cannot create task for {task_id} - no event loop")
                                    # Try to run task synchronously as fallback
                                    try:
                                        asyncio.run(self._run_task(task_id))
                                    except Exception as e:
                                        print(f"[FixedWindowsOrchestrator] Failed to run task {task_id} synchronously: {e}")
                                else:
                                    raise
                ...
                for task_id, task in self.tasks.items():
                    if task.enabled and task.status == AutomationStatus.IDLE and task.next_run and current_time >= task.next_run:
                        ...
                        try:
                            ...
                            if "no running event loop" in str(re):
                                print(f"[FixedWindowsOrchestrator] Cannot create scheduled task for {task_id} - no event loop")
                                # Try to run task synchronously as fallback
                                try:
                                    asyncio.run(self._run_task(task_id))
                                except Exception as e:
                                    print(f"[FixedWindowsOrchestrator] Failed to run scheduled task {task_id} synchronously: {e}")
                            else:
                                raise
    ...
    def _broadcast_status(self):
        ...
        status_data = {
            'type': 'fixed_orchestrator_status',
            'session_id': self.session_id,
            'timestamp': int(time.time() * 1000),
        ...
        try:
            self.client.set('scheduler_config/fixed_orchestrator_status', status_data)
        ...
        while self.running:
            try:
                ...
                heartbeat_data = {
                    ...
                    'fixed_orchestrator': True
                }
                
                self.client.set('scheduler_config/fixed_orchestrator_lock', heartbeat_data)
...
def main():
    """Main entry point for fixed Windows orchestrator"""
    parser = argparse.ArgumentParser(description='Fixed Windows-Compatible Automation Orchestrator')
    ...
    try:
        ...
        orchestrator = FixedWindowsOrchestrator(firebase_client, SimpleConsoleLogger("FixedWindowsOrchestrator"))
        ...
        print("\nFixed Windows Orchestrator stopped by user")
    except Exception as e:
