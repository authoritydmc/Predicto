...
CRICKET_ROLE_ID = os.getenv("DISCORD_ROLE_ID", "1492814845035417690")
...
def send_discord_notification(content, embed=None):
    ...
    if not webhook_url:
        log("[Discord] DISCORD_WEBHOOK_URL not set. Skipping notification.")
        return
    try:
        payload = {
            "content": content,
        ...
        log(f"[Discord] Notification sent (HTTP {resp.status_code}).")
...
def setup_firebase():
    try:
        default_val = os.getenv("FIREBASE_SERVICE_ACCOUNT")
        if not default_val:
            raise Exception("No FIREBASE_SERVICE_ACCOUNT")
        service_account_info = json.loads(default_val)
        ...
        log(f"Failed to configure Firebase: {e}")
        import sys
        sys.exit(1)
...
def scrape_cricbuzz_match(team_a, team_b, match_path=None):
    try:
        path = match_path
        if not path:
            log(f"[Scraper] Searching for match: {team_a} vs {team_b}...")
            ...
            for match in re.finditer(r'"matchId":(\d+)[\s\S]{0,1500}?"teamSName":"([A-Z]+)"[\s\S]{0,500}?"teamSName":"([A-Z]+)"', next_f_str):
                ...
                if (is_team_match(t1, team_a) and is_team_match(t2, team_b)) or \
                   (is_team_match(t1, team_b) and is_team_match(t2, team_a)):
                    ...
                    log(f"[Scraper] Strategy 1: Found matchId={found_match_id} for {team_a} vs {team_b} (teams: {match.group(2)} vs {match.group(3)})")
                    break
            ...
            if not found_match_id:
                ...
                if name_match:
                    ...
                    log(f"[Scraper] Strategy 2: Found matchId={found_match_id}")
                ...
                for match in re.finditer(r'href="(\/live-cricket-scores\/(\d+)\/([^"]+))"', live_html):
                    if lower_a in match.group(3) or lower_b in match.group(3):
                        ...
                        if lower_a in context.lower() and lower_b in context.lower():
                            path = match.group(1)
                            log(f"[Scraper] Strategy 3: Found path {path}")
                            break
            ...
            if not path and found_match_id:
                ...
                log(f"[Scraper] Constructed path: {path}")
...
def run_monitor():
    log("=== STARTING CRICBUZZ SCRAPER MONITOR (Python) ===")
    setup_firebase()
