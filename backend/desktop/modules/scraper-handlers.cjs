...
const registerScraperHandlers = () => {
  ...
  ipcMain.handle("scraper:run", async (_event, sport, matchId, teamA, teamB, scraperOrder, matchUrl) => {
    ...
    console.log(`[Scraper] Received params:`, { sport, matchId, teamA, teamB, scraperOrder, matchUrl });
    ...
    if (teamA) {
      args.push(teamA);
      console.log(`[Scraper] Added teamA: ${teamA}`);
    }
    if (teamB) {
      args.push(teamB);
      console.log(`[Scraper] Added teamB: ${teamB}`);
    }
    
    // Add match URL if provided (for direct scraping)
    if (matchUrl) {
      args.push("--match-url", matchUrl);
      console.log(`[Scraper] Added match URL: ${matchUrl}`);
    }
    
    // Add scraper sources priority
    if (scraperOrder) {
      args.push("--sources", scraperOrder);
      console.log(`[Scraper] Added scraper order: ${scraperOrder}`);
    }
    
    console.log(`[Scraper] Final args:`, args);
